...
const isOperator=(operatorStr:any):boolean=>{
switch(operatorStr){
  case '+':
  case '-':
  case '*':
...
const calculate=(firstNum:number,secondNum:number,operatorStr:string):number=>{
  switch(operatorStr){
    case '+':
...
console.log(colors.rainbow(`${firstNum} ${operatorStr} ${secondNum} is ${calculate(firstNum,secondNum,operatorStr)}`)) 
 }else console.log('something is not valid')
}
main()
